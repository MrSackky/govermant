...
export default function Home(props) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'x',
      render: (data) => <>
        <Button type="primary" size={"default"} onClick={() => gotoPage(data.question_id)} > แก้ไข </Button>
  ...
  const gotoPage = (id) => {
    console.log(id)
    Router.push('/admin/management/webboard/' + id);
  ...
  const onSearch = async (_search) => {
    ...
    const addactivitiesData = await apiInstance().get(
      'webboard?organization_id=' + (user ? user.organization_id : 0) + '&results=' + pagination.pageSize + '&page=1&search=' + _search + "&filters=" + filters
    );
  ...
  const fetch = async (params = {}) => {
    setLoading(true)
    const addactivitiesData = await apiInstance().get(
      'webboard?organization_id=' + (user ? user.organization_id : 0) + '&results=' + params.pagination.pageSize + '&page=' + params.pagination.current + "&search=" + search + "&sortField=" + params.sortField + "&sortOrder=" + params.sortOrder + "&filters=" + params.filters
    );
  ...
  const onDeleteOrganization = async () => {
    ...
    // }
    // console.log(data)
    const registerData = await apiInstance().delete('webboard/' + dataDelete.question_id, {});
  ...
  return (
    <Layout key={"home-webboard"} titlePage={"จัดการกระดานข่าว"} title="Government - Admin management" url={origin} origin={origin} isMain={true}
      indexSubMenu={"1"}
      indexMenu={"sub-1-4"}
      user={login} props={props} _routes={[
        {
          ...
          breadcrumbName: 'จัดการกระดานข่าว',
        }]}>
      <div>
        {contextHolder}
        <Row >
      ...
      >
        <p>คุณต้องการลบ "{dataDelete ? dataDelete.question_title : ""}" หรือไม่?  </p>
