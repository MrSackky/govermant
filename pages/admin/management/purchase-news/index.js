...
export default function Home(props) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'x',
      render: (data) => <>
        <Button type="primary" size={"default"} onClick={() => gotoPage(data.tender_id)} > แก้ไข </Button>
  ...
  const gotoPage = (id) => {
    console.log(id)
    Router.push('/admin/management/purchase-news/' + id);
  ...
  const onSearch = async (_search) => {
    ...
    const addactivitiesData = await apiInstance().get(
      'purchase-news?organization_id=' + (user ? user.organization_id : 0) + '&results=' + pagination.pageSize + '&page=1&search=' + _search + "&filters=" + filters
    );
  ...
  const fetch = async (params = {}) => {
    setLoading(true)
    const addactivitiesData = await apiInstance().get(
      'purchase-news?organization_id=' + (user ? user.organization_id : 0) + '&results=' + params.pagination.pageSize + '&page=' + params.pagination.current + "&search=" + search + "&sortField=" + params.sortField + "&sortOrder=" + params.sortOrder + "&filters=" + params.filters
    );
  ...
  const onDeleteOrganization = async () => {
    ...
    // }
    // console.log(data)
    const registerData = await apiInstance().delete('purchase-news/' + dataDelete.tender_id, {});
  ...
  return (
    <Layout key={"home-tender"} titlePage={"จัดการข่าวจัดซื้อจัดจ้าง"} title="Government - Admin management" url={origin} origin={origin} isMain={true}
      indexSubMenu={"1"}
      indexMenu={"sub-1-2"}
      ...
      <div>
        ...
        <Table
          columns={columns}
          rowKey={record => record.tender_id}
          dataSource={data}
      ...
      >
        <p>คุณต้องการลบ "{dataDelete ? dataDelete.tender_title : ""}" หรือไม่?  </p>
